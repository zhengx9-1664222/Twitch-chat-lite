...
import static room.CommandConstants.*;
...
public class SendThread extends Thread {
    ...
    public void join(String username, String channel) throws IOException {
        IRCMessage joinMessage = new IRCMessage(channel, username, JOIN, null);
        joinMessage.send(this.out);
    }

    public void channels(String username) throws IOException {
        IRCMessage channelMessage = new IRCMessage(ROOM_OVERRIDE, username, CHANNELS);
        channelMessage.send(this.out);
    }
    
    public String parseCommand(String content) {
        if (!content.startsWith(IRCConstants.COMMAND_PREFIX)) {
            return NONE;
        }
        String command = content.replace(IRCConstants.COMMAND_PREFIX, "").split(" ")[0];
        return command;
    }
