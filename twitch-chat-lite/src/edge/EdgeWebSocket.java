...
public class EdgeWebSocket {

    public static String LOG_PREFIX = "[EdgeWebSocket]: ";

    private Socket pubSubSocket;

    private ServerSocket edge;

    private ConcurrentHashMap<String, ChatWebSocket> clients;
    ...
    public EdgeWebSocket() throws Exception {
        this(true);
    }

    public EdgeWebSocket(boolean logResults) throws Exception {
        this.clients = new ConcurrentHashMap<String, ChatWebSocket>();
    ...
    private void addClient(String username, String channel, ChatWebSocket client) {
    ...
    public void run() {
        log("EdgeWebSocket launched on port: " + this.edge.getLocalPort());
        try {
            new Thread(() -> this.receive()).start();

            while (true) {
                Socket client = this.edge.accept();
                log("Found client: " + client.getLocalAddress());
                ChatWebSocket chatWebSocket = new ChatWebSocket(client);

                log("Connected to Client: " + client.getLocalAddress());
                new Thread(() -> this.clientHandler(chatWebSocket)).start();
            }
        } catch (Exception e) {
            ...
            while (true) {
                ...
                if (Room.ROOM_OVERRIDE.equals(message.getChannel())) {
                    ...
                    if (this.hasClient(sender)) {
                        ChatWebSocket client = this.clients.get(sender);
        ...
        for (String username : this.clients.keySet()) {
            try {
                if (channel.equals(this.memberships.get(username)) && !username.equals(message.getSender())) {
                    ChatWebSocket client = this.clients.get(username);
    ...
    private void clientHandler(ChatWebSocket client) {
        String username = null;
        try {
            //BufferedReader messageReader = new BufferedReader(new InputStreamReader(client.getInputStream()));
            //IRCMessage message = new IRCMessage(messageReader.readLine());

            IRCMessage message = new IRCMessage(new String(client.receive()));
            ...
            while (client.isConnected()) {
                ...
                message = new IRCMessage(new String(client.receive()));
    ...
    private void closeAllConnections() {
        ...
        for (ChatWebSocket client : this.clients.values()) {
            //SocketUtil.bestEffortClose(client);
            client.close();
        }
    }
