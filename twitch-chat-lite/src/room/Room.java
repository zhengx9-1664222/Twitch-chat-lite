...
public class Room {
    ...
    public boolean hasChannels() {
        return this.chatRooms.size() > 0;
    }

    private String getAllChatRooms() {
        StringBuilder builder = new StringBuilder();
        for (String room : this.chatRooms.keySet()) {
            if (room.equals(ROOM_OVERRIDE))
                continue;
            builder.append(room);
            builder.append(" ");
        }
        ...
        try {
            while (pubSub.isConnected()) {
                ...
                if (message.isCommand()) {
                    ...
                    if (command.equals(CommandConstants.CHANNELS)) {
                        this.channels(message);
                    } else if (command.equals(CommandConstants.JOIN)) {
                        this.join(message);
    ...
    private void channels(IRCMessage message) {
        CommandResponseCode code = Command.channels(this, message.getChannel(), message.getSender());
        if (code == CommandResponseCode.SUCCESS) {
            IRCMessage response = new IRCMessage(ROOM_OVERRIDE, message.getSender(), getAllChatRooms());

            log(message.getSender() + " requested to see channels");
            log("Broadcasting to PubSubs: " + response);

            sendToPubSubs(response);
        } else {
            String error = null;
            if (hasChatRoom(message.getChannel())) {
                error = message.getSender() + " is in channel: " + message.getChannel();
                error += ", All available channels are: " + getAllChatRooms();
            } else {
                error = "There are no running channels";
            }
            IRCMessage response = new IRCMessage(ROOM_OVERRIDE, message.getSender(), error);
            sendToPubSubs(response);
        }
    }
